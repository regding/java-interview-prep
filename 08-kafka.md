...
[⬅️ 上一章](07-elasticsearch.md) · [📖 返回目录](README.md) · [➡️ 下一章](09-rocketmq-rabbitmq.md)
...
> 🧭 初学者前置：00 章 §2.10（MQ 为什么存在）+ 16 章 §2 一致性模型更佳。

**📌 本章速览：核心要点**

1. Kafka 吞吐高的本质是「日志」：顺序写 + 页缓存 + 零拷贝（sendfile，数据从页缓存直达网卡不经用户态）+ 批量 + 压缩，读和写都不经过 JVM 堆；
