...
[⬅️ 上一章](02-jvm.md) · [📖 返回目录](README.md) · [➡️ 下一章](04-collections-src.md)
# 03 · 并发编程与 JUC 源码

> 适用对象：10+ 年经验的资深工程师 / 架构师候选人。本章要求「源码级理解」：AQS、线程池、ConcurrentHashMap 的关键流程要能徒手画出来，JMM 与锁升级要能讲清机制而不是背结论。
> 🧭 初学者前置：00 章 §2.5（线程与锁、竞态条件）；本章是全书最难章之一，卡壳属正常。

**📌 本章速览：核心要点**

1. JMM（Java 内存模型：规定多线程读写共享变量的可见性规则，见 §1.2）三大特性（原子/可见/有序）与 happens-before 是并发推理的公理系统，DCL 为什么必须 volatile 是试金石；
...
> 💡 初学者提示：happens-before 不是「时间上先发生」，而是「前一个操作的结果对后一个操作可见」的**保证**。下面每条都配一个具象场景帮助建立直觉——资深读者请直接背清单。

1. 程序顺序规则：单线程内书写在前的操作 happens-before 书写在后的；（场景：同一个线程里 `a=1` 写在 `b=a+1` 前面，读到的 a 必是 1——单线程世界永远讲道理）
2. 管程锁规则：解锁 happens-before 后续对同一把锁的加锁；（场景：线程 A 出了 `synchronized` 块前改的字段，线程 B 进同一块后必然看见新值）
3. volatile 规则：volatile 写 happens-before 后续对同一变量的读；（场景：配置开关 flag 设为 volatile，改完之后所有读线程立刻看到，不会读到缓存旧值）
4. 线程启动规则：`Thread.start()` happens-before 该线程内任何操作；（场景：start 前设置好的变量，子线程一定看得见）
5. 线程终止规则：线程内所有操作 happens-before 其他线程检测到它终止（join 返回）；（场景：工作线程算完结果退出，主线程 join 返回后一定能拿到计算结果）
6. 中断规则：`interrupt()` 调用 happens-before 被中断线程检测到中断；（场景：调用 interrupt 前写入的数据，被中断线程捕获异常后可见）
7. 终结器规则：对象构造完成 happens-before finalize 开始；
8. 传递性：A hb B 且 B hb C → A hb C。（场景：volatile 写 hb volatile 读 + volatile 读 hb 后续锁内操作 ⇒ volatile 写对锁内操作可见——规则可以串联）
...
> 💡 不用 volatile 会看到什么（初学者具象化）：极端情况下，线程 B 第一次检查 `instance != null` 通过、直接 `return instance`，然后调用 instance 的某个字段时发现是默认值（如 int 是 0）——对象「半成品」被发布。这个 bug 在低负载下可能几年不出现，压测或换 CPU 后突然爆发，这正是它经典的原因：**错误代码通常能跑，只是偶尔错得莫名其妙**。
...
**JDK 15（JEP 374）默认禁用并废弃偏向锁**：原因——1) 现代应用线程池场景下「单线程反复获取同一把锁」很少见，收益有限；2) 撤销偏向锁需要安全点 STW，撤销风暴反而制造停顿；3) 与 JIT 优化（如锁消除）冲突，维护成本高。**JDK 18 起彻底移除偏向锁实现**（JDK-8285835 删除 `UseBiasedLocking` 选项及全部相关代码）——所以现代 JDK 里关注「无锁 → 轻量 → 重量」即可，面试若被问到偏向锁，要能说明「它已在 JDK 18 被移除，是历史概念」。
...
解答：升级路径「无锁 → 轻量（CAS + 自旋）→ 重量（Monitor + park）」，核心思想是**乐观渐进**：根据竞争程度动态选择开销最小的方案，而不是一上来就阻塞。注意**偏向锁已在 JDK 18 彻底移除**，现代面试讲「无锁 → 轻量 → 重量」即可，不必再提偏向锁路径。废弃原因从收益与成本两端讲：收益端——偏向锁只在「单线程独占锁」场景有优势，而现代服务端（线程池、多线程框架）几乎都有竞争；成本端——撤销需要安全点 STW、批量重偏向逻辑复杂、与 JIT 锁消除互相干扰，维护/风险成本超过收益。面试加分：能点出「JDK 18 移除了偏向锁，这是历史概念」说明你对版本演进有体感。
...
- 方案：`StructuredTaskScope`（JDK 19 孵化 JEP 428 → 21 首预览 JEP 453 → 22/23/24 二三四次预览 → **25 第五次预览 JEP 505，截至 Java 25 仍是预览、未转正**）——任务在代码块内创建、块结束必须全部完成（或取消），子任务异常聚合到父任务；类似「并发版的 try-with-resources」；Java 25 起通过**静态工厂方法**（`StructuredTaskScope.open(...)`）而非公开构造器创建；与其配套的 `ScopedValue` 已在 **Java 25 JEP 506 转正**为标准 API（替代 ThreadLocal 的线程封闭方案，无 ThreadLocal 的内存泄漏与可继承污染问题）。
