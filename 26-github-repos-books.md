...
[⬅️ 上一章](25-interview-question-bank.md) · [📖 返回目录](README.md) · [➡️ 下一章](27-event-driven-cqrs.md)
# 26. 优质资源整合：GitHub 精选仓库 + 经典书单 + 学习路线

> 本章精选 GitHub 上与 Java 后端/分布式/系统设计/面试直接相关的优质仓库，
> 按「面试复习价值」分类整理；另附经典书单与刷题路线。标注 ⭐ 的为高优先级。
> 🧭 初学者提示：本章是「资深者的收藏夹」，初学者不必通读——直接跳 §七 经典书单的「三本入门排序」与 §八 刷题路线即可，其余等需要时再当索引用。
...
> 🧭 **初学者/转系读者先看这里——如果你只有时间读三本，按此顺序：**
> ① 《Java 核心技术·卷 I》（Cay Horstmann）——语法与标准库的地基，替代任何碎片化入门教程；
> ② 《Effective Java（第 3 版）》——写「对的 Java」的最佳实践，读完代码品味上一个台阶；
> ③ 《深入理解 Java 虚拟机（第 3 版）》——从「会用」到「懂平台」的分水岭，也是本系列 02 章的原著。
> 零基础者请再往前补一步：任意一本《Head First Java》或在线 Java 基础课程。以下书单默认你已跨过入门线。
