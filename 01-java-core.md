...
[⬅️ 上一章](00-prerequisite.md) · [📖 返回目录](README.md) · [➡️ 下一章](02-jvm.md)
# 01 · Java 语言核心进阶（资深向）

> 适用对象：10+ 年经验的资深工程师 / 架构师候选人。本章不讲 API 罗列，只讲原理、演进、权衡与工程踩坑。面试时"能讲出为什么"比"能背出是什么"值钱得多。
> 🧭 初学者前置：00 章 §2.1/§2.3/§2.4（编译与解释、注解与反射、集合与泛型）。
...
> 💡 初学者类比：把网络服务想成一家餐厅接单——**BIO** 是一位服务员只盯一桌客人（一连接一线程），1000 桌就要 1000 个服务员；**NIO** 是所有客人桌上装呼叫铃，一个服务员看总台灯亮再去哪桌（多路复用）；**AIO** 连「去端菜」都省了——后厨做好直接送到桌（系统完成读写后回调你）。C10K 问题 = 一万桌客人时 BIO 模式招不起服务员。
...
> 💡 初学者类比：序列化 = 把内存里的活对象「打包成快递」（字节流），好存储或网络传输；反序列化 = 收件后「照着清单重新组装」出对象。Java 原生打包方式（Serializable）自带完整说明书（类名+字段+类型），所以体积大、且说明书能被伪造（安全漏洞）；JSON/Protobuf 等则是更轻的打包格式，跨语言收件人也能拆包。
