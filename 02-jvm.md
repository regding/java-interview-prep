...
[⬅️ 上一章](01-java-core.md) · [📖 返回目录](README.md) · [➡️ 下一章](03-concurrency.md)
# 02 · JVM 原理与调优实战

> 适用对象：10+ 年经验的资深工程师 / 架构师候选人。本章聚焦「原理 → 参数 → 现象」的闭环：每个知识点都要能落到「线上出了什么问题、用什么命令看、怎么改参数」。
> 🧭 初学者前置：00 章 §2.1/§2.2（字节码与 JIT、GC 直觉），建议先跑通 §5 实验 1。

**📌 本章速览：核心要点**

1. 内存结构先分清「谁分配、谁回收」：堆管对象、栈管帧、元空间管类元数据、直接内存管 IO（不归 GC 管的堆外区域，见 00 章 §2.2 类比），指针压缩与逃逸分析决定对象到底占多大、分配在哪；
...
解答：热部署 = 新版本类由**新的类加载器实例**加载，旧加载器连同其加载的类整体可回收（类卸载的前提是「加载它的 ClassLoader 被 GC」）。所以：1) 必须自定义加载器，且每次部署 new 一个；2) 旧加载器不能被任何强引用持有——被 Spring 容器、ThreadLocal、静态字段引用都会导致泄漏（元空间撑爆的经典原因）；3) 系统/应用类加载器加载的类（JDK 类、classpath 根类）永远无法卸载。JDK 9+ 模块化后，强封装（JEP 396 默认禁止反射访问 JDK 内部）使热部署工具若要替换/访问 JDK 内部类，需显式 `--add-opens`；Spring DevTools 的策略是「只重载应用类（自定义 restart classloader），JDK/三方库留在 base classloader 不重载」，既避开模块封装限制，又保证热替换范围可控。原理不变，但工程上要理解模块封装这道边界。
...
> 💡 **深入排查**：本节覆盖了面试中最常见的排查场景。线上生产环境的完整排查工具箱（jcmd/JFR/Arthas/MAT/async-profiler）、OOM 全8种场景、内存泄漏 Top10 模式、CPU 热点深度分析、堆外内存与容器排查、以及真实故障案例集，请参阅 [33 · JVM 生产问题排查实战](33-jvm-production-troubleshooting.md)。
...
## 🧪 5 分钟实验：亲眼看一次 GC（初学者选做，资深读者跳过）

```shell
# 1. 写一个制造垃圾的小程序 Garbage.java
cat > Garbage.java <<'EOF'
import java.util.ArrayList;
import java.util.List;
public class Garbage {
    public static void main(String[] args) {
        for (int i = 0; i < 100_000; i++) {
            byte[] g = new byte[512 * 1024]; // 每次 512KB，绝大多数立刻变垃圾
        }
...
EOF
javac Garbage.java
# 2. 小堆运行并打开 GC 统一日志（JDK 9+；JDK 8 换 -XX:+PrintGCDetails）
java -Xms64m -Xmx64m -Xlog:gc* Garbage 2>&1 | head -20
# 3. 观察：GC 触发频率、每次回收了多少、耗时多少毫秒
# 4. 把 -Xmx64m 改成 -Xmx16m 再跑：GC 频率飙升——「堆越小 GC 越勤」肉眼可见
```

**看完本实验再读本章**：「分代」「停顿」不再是名词，而是日志里一行行真实发生的事件。
