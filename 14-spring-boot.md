...
[⬅️ 上一章](13-spring-core.md) · [📖 返回目录](README.md) · [➡️ 下一章](15-spring-cloud-mybatis.md)
...
> 🧭 初学者前置：13 章 Spring Core + 会用 Maven 建 Spring Boot 工程（00 章 §3）。
...
- [1. 自动装配源码](#1-自动装配源码)
- [2. 启动流程](#2-启动流程)
- [3. 自定义 Starter 开发](#3-自定义-starter-开发)
- [4. 配置体系](#4-配置体系)
- [5. 内嵌容器与生产就绪](#5-内嵌容器与生产就绪)
- [6. Spring Boot 3.x 新特性](#6-spring-boot-3x-新特性)
- [考点速查表](#考点速查表)
...
> 🧪 **5 分钟实验（初学者选做）**：`curl https://start.spring.io/starter.zip -d dependencies=web -d artifactId=demo -o demo.zip && unzip demo.zip && cd demo`，加一个 `@RestController` 返回字符串，然后 `./mvnw spring-boot:run`。启动日志里找三样东西：① `Starting DemoApplication ...`（主类入口）；② `Tomcat started on port 8080`（内嵌容器，没有装 Tomcat 它从哪来的？——带着问题读 §5.1）；③ 几秒内完成启动——对比传统 WAR 部署的流程差异，这就是「可执行 JAR + 内嵌容器」的体感。
